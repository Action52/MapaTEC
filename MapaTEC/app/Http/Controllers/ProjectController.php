...
class ProjectController extends Controller
{
    ...
    */
    public function show($id){
      ...
      /*$points = \DB::select(
        \DB::raw(
          "SELECT locations.id, points.name, points.geom
              FROM points, location_has_point, locations
              WHERE
              points.id = location_has_point.point_id AND
              locations.id = location_has_point.location_id
          "
          )
      );*/

      return \View::make('crudproyectos.show',compact('project', 'strategicpartners', 'users', 'campuses', 'categories', 'times', 'majors', 'courses'));
    }
